...
import type { VocabularyQuizProps, VocabularyMeaning } from '../data-types';

interface QuizItem {
  word: VocabularyQuizProps['vocabulary'][0];
...
const VocabularyQuiz: React.FC<VocabularyQuizProps> = ({
  ...
  const [stats, setStats] = useState({ correct: 0, incorrect: 0, knownWords: 0 });
  const [isInitialized, setIsInitialized] = useState(false);
  const [isCardFlipped, setIsCardFlipped] = useState(false);
  const [showChoices, setShowChoices] = useState(false);

  const generateOptions = (quizItem: QuizItem) => {
    if (!quizItem || !quizItem.meaning || !quizItem.word) {
      console.warn('Invalid quiz item provided');
      return;
    }

    const correctAnswer = quizItem.meaning.english;
    const currentDutchWord = quizItem.word.dutch;
    ...
    if (vocabulary && Array.isArray(vocabulary)) {
      vocabulary.forEach(word => {
        ...
        if (word && word.dutch && word.dutch !== currentDutchWord && word.meanings && Array.isArray(word.meanings)) {
          word.meanings.forEach(meaning => {
            if (meaning && meaning.english) {
              allWrongAnswers.push(meaning.english);
            }
          });
        }
      });
    ...
    if (allWrongAnswers.length < 5) {
      console.warn(`Not enough wrong answers available. Found ${allWrongAnswers.length}, need 5`);
  ...
  const selectNewWord = useCallback((shouldFreeze: boolean = true) => {
    ...
    const wordsToChooseFrom = (availableWords && availableWords.length > 0) ? availableWords : vocabulary;
    console.log('Words to choose from:', wordsToChooseFrom.length);
    ...
    const newItems = allQuizItems.filter(item => item.meaning.rating === 0);
    const highRated = allQuizItems.filter(item => item.meaning.rating >= 10 && item.meaning.rating <= 14);
    const midRated = allQuizItems.filter(item => item.meaning.rating >= 5 && item.meaning.rating <= 9);
    const lowRated = allQuizItems.filter(item => item.meaning.rating >= 1 && item.meaning.rating <= 4);
    ...
    if (random < 0.2 && newItems.length > 0) {
      ...
      selectedItem = allQuizItems[Math.floor(Math.random() * allQuizItems.length)];
    }
  ...
  }, [availableWords, vocabulary, onFreezeWord]);
  ...
  useEffect(() => {
    ...
    if (availableWords && availableWords.length > 0 && vocabulary && vocabulary.length > 0 && !isInitialized) {
      console.log('Initializing quiz...');
      // Only initialize the first word, don't freeze it yet
      selectNewWord(false);
      setIsInitialized(true);
    }
  }, [availableWords, vocabulary, isInitialized, selectNewWord]);
  ...
  const handleAnswerClick = async (answer: string) => {
    ...
    const isCorrect = answer === currentQuizItem.meaning.english;

    // Update ratings through parent component
    if (isCorrect) {
      await onUpdateRating(currentQuizItem.word.dutch, currentQuizItem.meaningIndex, 1);
    } else {
      await onUpdateRating(currentQuizItem.word.dutch, currentQuizItem.meaningIndex, -3);
    }
    ...
    setStats(prev => ({
      ...
      knownWords: vocabulary.reduce((count, word) =>
        count + word.meanings.filter(meaning => meaning.rating === 15).length, 0)
    }));
  };
  ...
  const nextWord = async () => {
    ...
    setTimeout(() => {
      selectNewWord(true); // This should freeze the new word
    }, 1000);
  };
  ...
  const resetQuiz = async () => {
    ...
    setTimeout(() => {
      if (vocabulary.length > 0) {
        selectNewWord(false); // Don't freeze on reset
        setIsInitialized(true);
      }
    }, 100);
  };
  ...
  const getOptionClass = (option: string) => {
    ...
    if (option === currentQuizItem?.meaning.english) {
  ...
  const getTotalKnownWords = () => {
    return vocabulary.reduce((count, word) =>
      count + word.meanings.filter(meaning => meaning.rating === 15).length, 0);
  };
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6 text-center">
        ...
        <div className="bg-blue-50 p-3 rounded-lg">
          <div className="text-2xl font-bold text-blue-700">{getTotalKnownWords()}</div>
          ...
          <div style={backCardStyle}>
            <div className="text-center">
              <div className="text-4xl font-bold mb-2">
                {currentQuizItem.meaning.english}
              </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg shadow-sm p-4 mb-4">
        <div className="text-center">
          <div className="text-sm text-amber-700 font-medium mb-1">Context Hint:</div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
        <div className="text-center text-gray-700">
          {!showResult && (
            <p className="text-lg font-medium text-indigo-700">What does "{currentQuizItem.word.dutch}" mean in English?</p>
          )}
        </div>
      </div>
      ...
      {showChoices && (
        <div
          className={`transition-all duration-500 ${showChoices ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        >
      ...
      {showResult && (
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6 transition-all duration-500 hover:shadow-xl">
          <div className="text-center mb-4">
            <div className={`text-2xl font-bold mb-2 ${selectedAnswer === currentQuizItem.meaning.english ? 'text-green-700' : 'text-red-700'
              }`}>
              {selectedAnswer === currentQuizItem.meaning.english ? '🎉 Correct!' : '❌ Incorrect'}
            </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-4">
            ...
            {currentQuizItem.meaning.examples.map((example, index) => (
              <div key={index} className="mb-3 last:mb-0">
                <div className="text-indigo-700 font-medium">🇳🇱 {example.nl}</div>
          ...
          {currentQuizItem.word.meanings.length > 1 && (
            <div className="bg-blue-50 p-4 rounded-lg mb-4">
              <h4 className="font-semibold text-blue-800 mb-3 text-lg">Other meanings of "{currentQuizItem.word.dutch}":</h4>
              {currentQuizItem.word.meanings
                .filter((_, index) => index !== currentQuizItem.meaningIndex)
                .map((meaning, index) => (
                  <div key={index} className="mb-2 last:mb-0">
                    <span className="font-medium text-blue-700">{meaning.english}</span>
                    <span className="text-blue-600 ml-2">({meaning.partOfSpeech})</span>
                    <span className="text-blue-500 ml-2">- {meaning.context}</span>
                  </div>
                ))}
            </div>
          )}
